...
#include <omp.h>
#endif
#pragma omp declare target to(ostei_s_s_s_s) device_type(any)

#pragma omp declare variant( ostei_s_s_s_s_target ) match(device={kind(nohost)})
int ostei_s_s_s_s(struct simint_multi_shellpair const P,
                  struct simint_multi_shellpair const Q, double screen_tol,
                  double *const restrict work,
                  double *const restrict INT__s_s_s_s) {
  SIMINT_ASSUME_ALIGN_DBL(work);
  SIMINT_ASSUME_ALIGN_DBL(INT__s_s_s_s);
  memset(INT__s_s_s_s, 0,
         P.nshell12_clip * Q.nshell12_clip * 1 * sizeof(double));
  ...
  // partition workspace
  SIMINT_DBLTYPE *const primwork =
      (SIMINT_DBLTYPE *)(work + SIMINT_NSHELL_SIMD * 0);
  ...
  int *offset_info = (int *)malloc(
      sizeof(int) * (SIMINT_SIMD_LEN + 1 + TopAM_size) * n_info_vector);
  ...
  for (ab = 0; ab < P.nshell12_clip; ++ab) {
    ...
    for (cd = 0; cd < Q.nshell12_clip; cd += SIMINT_NSHELL_SIMD) {
      const int nshellbatch = ((cd + SIMINT_NSHELL_SIMD) > Q.nshell12_clip)
                                  ? Q.nshell12_clip - cd
                                  : SIMINT_NSHELL_SIMD;
      int jend = jstart;
      ...
      for (j = jstart; j < jend; j += SIMINT_SIMD_LEN) {
        int *shelloffsets =
            offset_info + (SIMINT_SIMD_LEN + 1 + TopAM_size) * ivec;
        shelloffsets[0] = 0;
        ...
        if ((iprimcd + SIMINT_SIMD_LEN) >= nprim_icd) {
          ...
          if (iprimcd >= nprim_icd && ((icd + 1) < nshellbatch)) {
            ...
            shelloffsets[SIMINT_SIMD_LEN + 1 + 0] +=
                1; // for PRIM_PTR_INT__s_s_s_s
          }
          iprimcd++;
      ...
      for (i = istart; i < iend; ++i) {
        ...
        // const SIMINT_DBLTYPE Pxyz[3] = { SIMINT_DBLSET1(P.x[i]),
        // SIMINT_DBLSET1(P.y[i]), SIMINT_DBLSET1(P.z[i]) };
        ...
        for (j = jstart; j < jend; j += SIMINT_SIMD_LEN) {
          ...
          // int lastoffset = 0;
          const int nlane =
              (((j + SIMINT_SIMD_LEN) < jend) ? SIMINT_SIMD_LEN : (jend - j));
          ...
          */
          ...
          if (shelloffsets[SIMINT_SIMD_LEN + 1])
            PRIM_PTR_INT__s_s_s_s += 1;
          ivec++;
          ...
          if (check_screen) {
            ...
            prim_screen_res =
                SIMINT_MUL(bra_screen_max, SIMINT_DBLLOAD(Q.screen, j));
...
#ifdef SIMINT_PRIM_SCREEN_STAT
          calc_nvec++;
          int calc_nprim_in_vec =
              count_prim_screen_survival(prim_screen_res, screen_tol);
          ...
          const SIMINT_DBLTYPE one_over_PQalpha_sum =
              SIMINT_DIV(const_1, PQalpha_sum);
          ...
          const SIMINT_DBLTYPE alpha =
              SIMINT_MUL(PQalpha_mul, one_over_PQalpha_sum); // alpha from MEST
          ...
          const SIMINT_DBLTYPE one_over_2pq =
              SIMINT_MUL(one_half, one_over_PQalpha_sum);
          ...
          if (lastoffset == 0) {
            ...
            if ((ca_cnt > 0) && (new_idx == ca_res_idx[ca_cnt - 1])) {
              ca_buf[ca_cnt - 1] =
                  _mm512_add_pd(ca_buf[ca_cnt - 1], PRIM_INT__s_s_s_s[0]);
            } else {
...
#ifdef SIMINT_PRIM_SCREEN_STAT
  double *eri_res_end_pos =
      INT__s_s_s_s + 1 * P.nshell12_clip * Q.nshell12_clip;
...
int ostei_s_s_s_s_target(struct simint_multi_shellpair const P,
                  struct simint_multi_shellpair const Q, double screen_tol,
                  double *const restrict work,
                  double *const restrict INT__s_s_s_s) {

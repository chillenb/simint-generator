...
extern "C" {
    ...
    static inline void contract_device(int ncart,
                  int const * restrict offsets,
                  double const * restrict src,
                  double * restrict dest);

    #pragma omp declare variant( contract_device ) match(device={kind(nohost)})
    static inline
    ...
    static inline void contract_device(int ncart,
                  ...
                  double * restrict dest)
    {
        double const * restrict src_tmp = src;
        double * restrict dest_tmp = dest + offsets[0];

        for(int np = 0; np < ncart; ++np)
        {
            dest_tmp[np] += src_tmp[np];
        }
    }

    static inline void contract_all_device(int ncart, double const * restrict src, double * restrict dest);
    
    #pragma omp declare variant( contract_all_device ) match(device={kind(nohost)})
    static inline
    void contract_all(int ncart,
                      double const * restrict src,
                      double * restrict dest)
    {
        //printf("contracting on host\n");
        int offsets[1] = {0};

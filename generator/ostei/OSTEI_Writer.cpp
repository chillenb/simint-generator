...
std::string OSTEI_Writer::OMPTargetFunctionName_(QAM am) const
{
    return StringBuilder("ostei_",
                         amchar[am[0]], "_",
                         amchar[am[1]], "_" ,
...
std::string OSTEI_Writer::OMPTargetFunctionPrototype_(QAM am) const
{
    std::string fname = OMPTargetFunctionName_(am);
    std::string indent(fname.length()+1+4, ' '); // +4 for return type
...
void OSTEI_Writer::Write_Permute_(QAM am, bool swap12, bool swap34) const
{
    ...
    if(use_openmp_target_) {
        os_ << "#pragma omp declare target to( " << FunctionName_(permuted) << " ) device_type(any)\n";
        os_ << "#pragma omp declare variant ( " << OMPTargetFunctionName_(permuted) << " ) match(device={kind(nohost)})\n";
    }
    ...
    // TARGET VERSION
    if(use_openmp_target_) {
        os_ << OMPTargetFunctionPrototype_(permuted) << "\n";
        os_ << "{\n";
        ...
        os_ << "}\n";
        os_ << "\n";
        osh_ << OMPTargetFunctionPrototype_(permuted) << ";\n\n";
    }
}
...
void OSTEI_Writer::PartitionWorkspace(void) const
{
    ...
    if(info_.UseStack())
    {
        for(const auto & am : hrr_writer_.Algo().GetIntermediates())
...
void OSTEI_Writer::Write_Full_(void) const
{
    ...
    if(use_openmp_target_) {
        // declare that the ostei function can run on both device and host
        os_ << "#pragma omp declare target to( " << FunctionName_(am) << " ) device_type(any)\n\n";

        // if running on the device, use the device-optimized version
        os_ << "#pragma omp declare variant( " << OMPTargetFunctionName_(am) << " )  match(device={kind(nohost)})\n";
    }
    ...
    ///////////////////////////////////
    // OSTEI FUNCTION: TARGET VERSION
    ///////////////////////////////////


    if(use_openmp_target_) {
        os_ << OMPTargetFunctionPrototype_(am) << "\n";
        os_ << "{\n";
        os_ << "\n";
...
void OSTEI_Writer::WriteFile(void) const
{
    ...
    if(use_openmp_target_) {
        osh_ << OMPTargetFunctionPrototype_(am) << ";\n\n";
    }
